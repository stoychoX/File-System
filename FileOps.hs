...
replacePath :: FileSystem -> [FileSystem] -> [FileSystem]
replacePath new@(Root name xs) xs' = new : filter (not . isNameOfFolder name) xs' 
...
add path toAdd (Just old@(Root n xs)) = case getNextDir path of 
    ...
    Just (rest, curr) -> case changeDir curr xs of 
        Nothing -> Nothing 
        Just (Root path' files) -> case add rest toAdd (Just (Root path' files)) of
            Nothing -> Nothing
